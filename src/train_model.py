# =============================================
# src/train_model.py  (Version Finale - Sauvegarde l'encodeur)
# =============================================
...
def build_encoder(X_train: pd.DataFrame) -> ColumnTransformer:
    """Construit et fit l'encodeur sur X_train."""
...
def encode_features(X_train, X_test):
    """Encode les catégorielles et retourne les DataFrames + l'encodeur fitté."""
    cat_cols = X_train.select_dtypes(include=['object', 'category']).columns.tolist()
    ...
    # Deuxième passe anti-leakage sur colonnes encodées
    to_drop = [c for c in X_train_enc.columns
    ...
    # ── SAUVEGARDE DE L'ENCODEUR ── clé pour que Flask fonctionne
...
def train_random_forest(X_train_enc, X_test_enc, y_train, y_test):
    ...
    plt.ylabel("Vraie Valeur"); plt.xlabel("Prédiction")
    ...
    print("💾 RandomForest sauvegardé → models/randomforest_churn.pkl\n")
    return best_rf
...
def run_training():
    ...
    # Clustering (avant encodage, sur données brutes numériques)
    clustering(X_train)

    # Encodage — sauvegarde encoder.pkl pour Flask
    X_train_enc, X_test_enc, encoder = encode_features(X_train, X_test)

    # Diagnostic post-encodage
    diagnostic(X_train_enc, y_train)

    # Classification
    train_random_forest(X_train_enc, X_test_enc, y_train, y_test)

    print("🎉 MODÉLISATION TERMINÉE !")
    print("   Modèles dans models/ : randomforest_churn.pkl, kmeans_model.pkl, encoder.pkl")
